import { _decorator, Component, Node, Prefab, Vec3, Sprite, SpriteFrame, ImageAsset, UITransform, Texture2D, Canvas, game } from 'cc';
...
export class GameManager extends Component {
     ...
     * 直接设置倍数（GM 一键完成订单用，根据目标等级智能选择倍率）

     * @param multiplier 目标倍率（1 | 2 | 4）

     */

    public setMultiplier(multiplier: number): void {

        const index = GameManager.MULTIPLIERS.indexOf(multiplier);

        if (index < 0) {

            console.warn(`[GameManager] setMultiplier: invalid multiplier ${multiplier}`);

            return;

        }

        if (this._multiplierIndex === index) {

            return;

        }

        this._multiplierIndex = index;

        console.log(`[GameManager] Multiplier set -> x${this.currentMultiplier}`);
    ...
    onLoad() {
        ...
        // 限制帧率为 60fps，避免高刷新率显示器下动画变快
        // 注意：Cocos Creator 3.8 无 director.setFrameRate，使用 game.frameRate
        game.frameRate = 60;
